...
bool PortVal::IsIPSEC() const
	{
	return (val.uint_val & PORT_SPACE_MASK) == IPSEC_PORT_MASK;
	}
...
const PortValPtr& ValManager::Port(uint32_t port_num) const
	...
	else if ( mask == IPSEC_PORT_MASK )
		return Port(port_num, TRANSPORT_IPSEC_NAIVE);
	else
