...
## IPSec 中 AH/ESP 连接的超时时间，用于清理 IPSec 连接状态。
## （供 IPSecNaive Analyzer 使用）
const ipsec_naive_inactivity_timeout = 2 min &redef;
...
export {
	type Certificate: record {
		...
		modulus: string &optional &log;
		x: string &optional &log;
		y: string &optional &log;
		raw_pkey: string &optional &log;
	};
