...
refine connection Handshake_Conn += {
	...
	function proc_certificate_request(cont: bytestring) : bool
		%{
		// 发送一个 ssl_certificate_request 事件	
		zeek::BifEvent::enqueue_ssl_certificate_request(zeek_analyzer(), zeek_analyzer()->Conn(),
		   	   zeek::make_intrusive<zeek::StringVal>(cont.length(),
...
refine typeattr CertificateRequest += &let {
	proc : bool = $context.connection.proc_certificate_request(cont);
};
