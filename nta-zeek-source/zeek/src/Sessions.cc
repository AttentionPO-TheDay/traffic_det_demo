...
NetSessions::~NetSessions()
	...
	for ( const auto& entry : icmp_conns )
		Unref(entry.second);
	for ( const auto& entry : ipsec_conns )
		Unref(entry.second);
...
void NetSessions::ProcessTransportLayer(double t, const Packet* pkt, size_t remaining)
	...
	case IPPROTO_ESP:
		...
		// 如果不加这一行，那么来自两边的 ESP/AH 会被拆成两条流，每条流里只有一边的数据
		id.is_one_way = false;
...
void NetSessions::Remove(Connection* c)
	...
	if ( c->IsKeyValid() )
		...
		case TRANSPORT_IPSEC_NAIVE:
			if ( ipsec_conns.erase(key) == 0 )
				reporter->InternalWarning("connection missing");
			break;
...
void NetSessions::Insert(Connection* c)
	...
	case TRANSPORT_IPSEC_NAIVE:
		old = LookupConn(ipsec_conns, c->Key());
		ipsec_conns.erase(c->Key());
		InsertConnection(&ipsec_conns, c->Key(), c);
		break;
...
bool NetSessions::IsLikelyServerPort(uint32_t port, TransportProto proto) const
	...
	if ( proto == TRANSPORT_TCP )
		...
		port |= ICMP_PORT_MASK;
	else if ( proto == TRANSPORT_IPSEC_NAIVE )
		port |= IPSEC_PORT_MASK;
...
unsigned int NetSessions::MemoryAllocation()
	...
	return ConnectionMemoryUsage()
		...
		+ (ipsec_conns.size() * (sizeof(ConnectionMap::key_type) + sizeof(ConnectionMap::value_type)))
